...
Dependencies are split into Python and Node.js parts.
...
Notes:
- `scripts/html2pptx.js` supports `playwright` *or* `playwright-core`. If you use `playwright-core`, it will try to launch system Chrome via `channel: "chrome"`.

### System tools (optional but common)

- **LibreOffice** (PPTX↔PDF conversions used by thumbnail tools)
